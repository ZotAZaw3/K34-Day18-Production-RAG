...
def _split_by_size(text: str, size: int) -> list[str]:
    """Split text into word-preserving pieces each ≤ size chars."""
    pieces, current = [], ""
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    def _flush(parent_text: str) -> None:
        pid = f"parent_{len(parents)}"
        parents.append(Chunk(text=parent_text,
                             metadata={**metadata, "chunk_type": "parent", "parent_id": pid}))
        for piece in _split_by_size(parent_text, child_size):
            children.append(Chunk(text=piece,
